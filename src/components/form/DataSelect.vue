...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      vField () {
        return this.dict != null ? 'itemCode' : this.valField;
      },
      tField () {
        return this.dict != null ? 'itemValue' : this.textField;
      }
    },
    methods: {
      /**
      ...
      queryData () {
        ...
        this.$http.apiGet(url, param).then((res) => {
          ...
          if (res.code!=200) {
    		...
    		this.mData = utils.constructSelect(res.datas, textField, valField);
          }
        });
      },
