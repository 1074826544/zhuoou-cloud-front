        ...
        <template v-else>
          <FormItem :key="'formItem'+index"
            ...
            <template v-else-if="item.type==='imgUpload'">
             ...
             <!--图标选择-->
            <template v-else-if="item.type==='iconSelect'">
              <IconSelect v-model="fromData[item.name]"
                         :data="item"
                         :readonly="item.readonly"
                		 :disabled="item.disabled"
                         @on-click="(val) =>{ if( item.onClick!==null){item.onClick(fromData,mData)}}">
...
<script>
  import {TreeSelect, DataSelect, ImgUpload, FormEditor, IconSelect} from 'components/';
  ...
  export default {
      ...
      initDict (data) {
    	  for (let i in data) {
    		  for (let j in data[i]) {
    			 let obj = data[i][j];
    			 //字典处理
	             if(isNotBlank(obj.dict)){
	            	if(obj.type==='radio'||obj.type==='checkbox'){
	            		let url = config.api.dict + obj.dict;
	            		let param = {simple: true}
	            		this.$http.apiGet(url, param).then((res) => {
	            			if (res.code!=200) {
	            				this.$http.handleError(res);
	            			} else {
	            				this.$set(obj,'data',utils.constructSelect(res.datas, "itemValue", "itemCode"));
	            			}
	            		});
	            	}
	              }
              ...
              if (obj != null && data[name] != null) {
                ...
                if (obj.type == 'datetimerange') {
                  if (data[name].length == 2 && data[name][0] instanceof Date && data[name][1] instanceof Date) {
                    ...
                    data[name] = arr;
                    //data[name]=arr[0]+","+arr[1];
                  }
                }
                if(obj.type == 'select' && utils.isArray(data[name])){
    ...
    components: {TreeSelect, DataSelect, ImgUpload, FormEditor,IconSelect}
  };
</script>
