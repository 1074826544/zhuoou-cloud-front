        ...
        this.$http.apiGet(this.dataUrl, param).then((res) => {
          ...
          if (res.code!=200) {
        	...
        	if(res.datas.records){
        		this.$emit('on-data-loaded', res.datas.records, res);
        		if (this.constructTree) {
        			this.treeData = utils.constructTree(res.datas.records, 'name', false, res.datas.selected);
        		} else {
        			this.treeData = res.datas.records;
        		}
        		this.mData = res.datas.records;
        	}else{
        		this.$emit('on-data-loaded', res.datas, res);
        		if (this.constructTree) {
        			this.treeData = utils.constructTree(res.datas, 'name', false, res.datas.selected);
        		} else {
        			this.treeData = res.datas;
        		}
        		this.mData = res.datas;
        	}
          }
        });
      },
      onSelectChange (slected) {
        if (isNotBlank(slected) && slected.length > 0) {
