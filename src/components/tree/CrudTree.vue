    ...
    <div class="tree-toolbar" v-if="treeOptions.showToolbar">
      ...
      <template v-if="this.treeOptions.editOptions">
        <PermsValid :perms-name="createPermsTree" v-if="treeOptions.createUrl">
          <Button size="small" type="primary" shape="circle" icon="md-add" @click="treeAddData()">添加</Button>
        </PermsValid>
        <PermsValid :perms-name="updatePermsTree" v-if="treeOptions.updateUrl">
          <Button type="info" size="small" shape="circle" icon="ios-create" @click="treeEditData()"
      ...
      <PermsValid :perms-name="deletePermsTree" v-if="treeOptions.deleteUrl" style="padding-top: 4px">
        <Poptip confirm title="您确认删除选中的内容吗？" @on-ok="treeDelData()" placement="bottom-start" transfer>
          <Button type="warning" size="small" shape="circle" icon="ios-trash" @click="" :disabled="!treeOptions.selected">
            删除
          </Button>
        </Poptip>
      </PermsValid>

      <Button v-if="treeOptions.manageUrl" size="small" shape="circle" icon="md-person" 
              @click="$router.push(treeOptions.manageUrl);">角色管理
      </Button>
    </div>
    <!--是否显示搜索框-->
    <Input v-if="treeOptions.showSearch" v-model="condition" suffix="ios-search" @on-change="search" placeholder="输入搜索信息..."/>
    <!--顶部工具栏结束-->
    <DataTree ref="tree"
    ...
    methods: {
      ...
      search () {
    	if(this.treeOptions.showSearch){
    		if(!isNotBlank(this.treeOptions.param)){
    			this.treeOptions.param={};
    		}
    		if(isNotBlank(this.condition)){
    			this.treeOptions.param.condition=this.condition;
    		}else{
    			this.treeOptions.param.condition=null;
    		}
    		this.$refs.tree.queryData();
    	}
      },
      ...
      treeDelData () {
        ...
        if (selected != null) {
          ...
          this.$http.apiDelete(this.treeOptions.deleteUrl, {ids: selected.id}).then((res) => {
            this.loading = false;
            if (res.code!=200) {
