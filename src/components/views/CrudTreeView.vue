    ...
    <i-col v-if='treeOptions' :xs='8' :sm='6' :md='6' :lg='4' class='left-col'>
      ...
      <div class='tree-toolbar'>
        <PermsValid :perms-name='createPerms'>
          <Button size='small' shape='circle' type='primary' icon='plus' @click='showCreate'>添加</Button>
        </PermsValid>

        <PermsValid :perms-name='deletePerms'>
          <Poptip confirm title='您确认删除选中的内容吗？' @on-ok='treeDelData()' placement='bottom-start' transfer>
            <i-button size='small' type='primary' shape='circle' icon='trash-b' :disabled='!treeOptions.selected'>删除
            </i-button>
          </Poptip>
        </PermsValid>
      </div>
      ...
      <template v-if='treeOptions.selected || treeOptions.createMode'>
        ...
        <div style='padding:10px'>
          <div v-show='treeOptions.createMode'>
            <PermsValid :perms-name='createPerms'>
              <Button shape='circle' type='primary' icon='plus' @click='treeAddData()'>保存添加</Button>
            </PermsValid>
          </div>
          <div v-show='treeOptions.selected && !treeOptions.createMode'>
            <PermsValid :perms-name='updatePerms'>
              <i-button type='primary' shape='circle' icon='edit' @click='treeEditData()'>保存修改</i-button>
            </PermsValid>
          </div>
        </div>
