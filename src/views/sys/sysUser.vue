  ...
  <CrudView ref='crudView' :tableOptions='tableOptions' :treeOptions='treeOptions'>
    ...
    <template slot='toolbar'>
      ...
      <Poptip confirm title='您确认重置当前选中账号的密码吗？' @on-ok='roleResPassword()' placement='bottom-start' transfer>
        <i-button icon='md-unlock' :disabled='isSingle'>重置密码
        </i-button>
      </Poptip>
...
<script>
  ...
  const tableEditOptions = {
    ...
    dynamic: [
      [
        ...
        {name: 'mobile', type: 'text', span: 12, label: '手机号码'}
      ], [
        {
          name: 'roleId',
          type: 'select',
  ...
  const tableOptions = {
    ...
    title: '用户',
    permsPrefix: 'sysUser',
    dataUrl: '/sys/sysUser/list',
    createUrl: '/sys/sysUser/create',
    deleteUrl: '/sys/sysUser/delete',
    ...
    columns: [
      ...
      {key: 'mobile', title: '手机号码', width: 150},
  ...
  export default {
    ...
    methods: {
      roleResPassword() {
        ...
        if (selection != null) {
          this.$http.apiPut('/sys/sysUser/resetPwd', {id: selection[0].id}).then((res) => {
            this.loading = false;
            if (!res.code==200) {
