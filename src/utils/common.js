...
export default {
  ...
  constructTree: function (data, name, expand) {
	...
	if(!this.isEmptyObject(data)){
		for (let i in data) {
			if (data[i].parentId != '0' && !this.isEmptyObject(data[i].parentId)) {
				for (let j in data) {
  ...
  changeColor(num){
	...
	document.getElementById('app').parentNode.className ='theme'+num ;
	//存储localStoarge，用于进入系统时，记住用户上一次的选择，自动加载用户上一次选择的主题主题，记得在mounted()里面调用
	localStorage.setItem('app',document.getElementById('app').parentNode.className)
  }
};
