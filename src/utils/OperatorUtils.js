...
const OperatorUtils = {
  ...
  setBaseData(data) {
    Lockr.set(prefix + 'token', config.authPre+data.authData.token);
  ...
  getMain() {
    if (this.main === null) {
      let main = null;
      if(isNotBlank(this.getUserData()) && isNotBlank(this.getUserData().main)){
    	  main=JSON.parse(this.getUserData().main);
      }
